...
def processTextFixed(sentences, stopIndex):
    stop_words = set(stopwords.words('english')) 
    newSentence = ''
    for indx, info in enumerate(sentences):
        if indx < stopIndex:
            #print(indx, info, type(info))
            newSentence += str(info) 
            newSentence += ' '
    
    newSentence = re.sub('[^A-Za-z0-9 ]+', '', newSentence)
